...
> **バージョン:** `version.py` の `__version__` で一元管理（現行 **v3.9.1**）。MSI ビルドと自動更新チェックがこの値を参照する。
...
4. 本番サーバへ MSI を scp（例: `WonderLinko_3.9.1.msi`）
5. **v3.2.3 以降:** `desktop_app_releases/` は bind mount のため **git pull + scp で即反映**（通常 deploy 不要）

```json
{
  "version": "3.9.1",
  "url": "https://wl-ai-board.internal.wonder-link.com/api/bs/desktop-app/WonderLinko_3.9.1.msi"
}
```
